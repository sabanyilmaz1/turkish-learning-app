...
export const useFlashcards = (cards: Flashcard[], startWithFrench: boolean = true) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isFlipped, setIsFlipped] = useState(false);
  const [mode, setMode] = useState(startWithFrench);
  const [flashcardsProgress, setFlashcardsProgress] = useState<{ [key: string]: number }>({});
  ...
  const filteredCards = cards.filter(
    card => !flashcardsProgress[card.front] || flashcardsProgress[card.front] < MAX_SUCCESS_COUNT
  );
